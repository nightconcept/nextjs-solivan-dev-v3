...
describe('TagPage', () => {
  ...
  it('renders the correct heading for the tag', async () => {
    ...
    // Assert
    await waitFor(() => {
        expect(screen.getByRole('heading', { level: 1, name: `Posts tagged with "${formattedTag}"` })).toBeInTheDocument();
    });
  });
  ...
  it('displays posts correctly when posts with the tag exist', async () => {
    ...
    await waitFor(() => {
        ...
        expect(screen.getByRole('link', { name: 'Post Two' })).toHaveAttribute('href', '/blog/post-2');
    });
  });
  ...
  it('handles tags with hyphens correctly', async () => {
    ...
    await waitFor(() => {
        // Check heading
        expect(screen.getByRole('heading', { level: 1, name: `Posts tagged with "${formattedHyphenatedTag}"` })).toBeInTheDocument();
        // Check breadcrumb
