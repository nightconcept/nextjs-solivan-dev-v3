...
describe("TagPage", () => {
  ...
  it("displays posts correctly when posts with the tag exist", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByRole("link", { name: "Post Three" }),
  ...
  it('displays "No posts found" message when no posts match the tag', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No posts found with this tag."),
      ).toBeInTheDocument();
  ...
  it("handles tags with hyphens correctly", async () => {
    ...
    await waitFor(() => {
      ...
      // Check breadcrumb
      expect(
        screen.getByText(`Tag: ${formattedHyphenatedTag}`),
      ).toBeInTheDocument();
      // Check post is displayed
      expect(
        screen.getByRole("link", { name: "Post Four" }),
      ).toBeInTheDocument();
    });
  });
});
