...
All in all, I want to take these next four years as a chance to grow and do something productive for me. I will not wallow in loss.`,
    ...
    // Assertions
    // Check Title
    expect(screen.getByRole("heading", { level: 1, name: /A New Path/i }))
      .toBeInTheDocument();
    ...
    expect(screen.getByText(/toxic doom scrolling and internet/i))
      .toBeInTheDocument();
  ...
  it("should call notFound if post does not exist", async () => {
    ...
    await expect(BlogPostPage({ params: Promise.resolve({ slug }) })).rejects
      .toThrow(
