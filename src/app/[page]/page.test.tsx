...
describe("Page Component - /app/[page]/page.tsx", () => {
  ...
  it("should render the page content correctly when found", async () => {
    ...
    expect(
      screen.getByRole("link", { name: MOCK_ABOUT_PAGE.frontmatter.title }),
    ).toHaveAttribute(
      "href",
      `/${slug}`,
    );
    ...
    expect(screen.getByText("This is the content of the about page."))
      .toBeInTheDocument();
  ...
  it("should use capitalized slug as title if frontmatter title is missing", async () => {
    ...
    expect(screen.getByRole("heading", { level: 1, name: "Missing-title" }))
      .toBeInTheDocument();
    // Expect breadcrumb to also use capitalized slug
    expect(screen.getByRole("link", { name: "Missing-title" })).toHaveAttribute(
      "href",
      `/${slug}`,
    );
  });
...
describe("generateStaticParams - /app/[page]/page.tsx", () => {
  ...
  it("should generate static params based on all markdown pages", async () => {
    ...
    expect(staticParams).toEqual([{ page: MOCK_ABOUT_PAGE.slug }, {
