...
describe("TagPage", () => {
  ...
  it.todo("displays posts correctly when posts with the tag exist", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole("link", { name: "Post One" }))
        .toBeInTheDocument();
      expect(screen.getByRole("link", { name: "Post Two" }))
        .toBeInTheDocument();
      expect(screen.queryByRole("link", { name: "Post Three" })).not
        .toBeInTheDocument();
  ...
  it('displays "No posts found" message when no posts match the tag', async () => {
    // Arrange
    const mockPosts = [
      createMockPost("post-3", "Post Three", ["different-tag"]),
    ];
    vi.mocked(getAllPosts).mockReturnValue(mockPosts);
    ...
    // Assert
    await waitFor(() => {
      expect(screen.getByText("No posts found with this tag."))
        .toBeInTheDocument();
  ...
  it("handles tags with hyphens correctly", async () => {
    ...
    const mockPosts = [
      createMockPost("post-4", "Post Four", ["another-cool-tag"]),
    ];
    vi.mocked(getAllPosts).mockReturnValue(mockPosts);
    ...
    await waitFor(() => {
      ...
      // Check breadcrumb
      expect(screen.getByText(`Tag: ${formattedHyphenatedTag}`))
        .toBeInTheDocument();
      // Check post is displayed
      expect(screen.getByRole("link", { name: "Post Four" }))
        .toBeInTheDocument();
    });
  });
});
