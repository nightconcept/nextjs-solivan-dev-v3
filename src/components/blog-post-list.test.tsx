...
import { act, fireEvent, render, screen, within } from "@testing-library/react"; // Import act and within
...
const createMockPost = (
  ...
  excerpt:
    `This is the excerpt for test post ${id}. It should be around 150 characters long to test the truncation properly. Lorem ipsum dolor sit amet, consectetur adipiscing elit.`,
...
const mockPosts: PostMetadata[] = Array.from(
  { length: 10 },
  (_, i) => createMockPost(i + 1, `2024-01-${10 - i}`, (i + 1) % 3 === 0), // Posts 3, 6, 9 are drafts
);
...
describe("BlogPostList Component", () => {
  ...
  it("renders post details correctly (title, excerpt, date, read time, author)", () => {
    ...
    expect(
      getByText(
        post1.dateObject.toLocaleDateString("en-US", {
          year: "numeric",
          month: "long",
          day: "numeric",
        }),
  ...
  it('shows "See More" link when showSeeMore is true and there are more published posts than postsPerPage', () => {
    ...
    expect(screen.getByText("See More").closest("a")).toHaveAttribute(
      "href",
      "/blog",
    );
  });
  ...
  it('does not show "See More" link when showSeeMore is false', () => {
    render(
      <BlogPostList posts={mockPosts} postsPerPage={3} showSeeMore={false} />,
    );
  ...
  it('does not show "Next" button on the last page of published posts', () => {
    ...
    const totalPublishedPages = Math.ceil(
      mockPosts.filter((p) => !p.draft).length / postsPerPage,
    ); // Should be 3
    render(
      <BlogPostList
        posts={mockPosts}
        postsPerPage={postsPerPage}
        page={totalPublishedPages}
      />,
    );
  ...
  it('shows "Prev" button when not on the first page (using published posts)', () => {
    ...
    expect(screen.getByText("« Prev").closest("a")).toHaveAttribute(
      "href",
      "/blog",
    ); // Page 2 links back to /blog (page 1)
  });
